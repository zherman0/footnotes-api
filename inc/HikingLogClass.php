...
class HikingLogClass extends DataBaseMysqli
{
	...
	{
		$query = "SELECT tbl.hikeId, tbl.userId, tbl.locationId, loc.name, user.username, tbl.hikeDate, tbl.description FROM hikingLog tbl ";
		$query .= " JOIN hikingLocations loc on tbl.locationId = loc.locationId JOIN user on user.userId = tbl.userId";
	...
	function saveHike($_data, $_id = null)
	{
		...
		if (($_data["userId"] != NULL) && ($_data["locationId"] != NULL)) // at least the user and location must be set
		{
			...
			if ($_id != null) {
				$sql_update .= " WHERE hikeId=$_id";
			}
			$stmt = $this->DBconn->prepare($sql_update);
			if ($stmt == false) {
				return "Error with $action: $sql_update";
			}
			$truncateDescription = substr($_data["description"], 0, 64000);
			$stmt->bind_param(
				"ssss",
				$_data["userId"],
				$_data["locationId"],
